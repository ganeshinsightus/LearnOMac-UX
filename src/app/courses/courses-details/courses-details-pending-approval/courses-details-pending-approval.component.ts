...
@Component({
  templateUrl: "./courses-details-pending-approval.component.html",
  styleUrls: ["./courses-details-pending-approval.component.scss"]
})
export class CoursesDetailsPendingApprovalComponent {
    ...
    },
    {
      ...
      cellsRenderer: (row: number, column: any, value: any): string => {
        return (
          `<div data-row='` +
          row +
          `' class='viewButton' style='color:white;background-color:green;margin-left: 50px;'></div>`
        );
      }
    }
  ];
  ...
  rendered = (): void => {
    let uglyviewButtons = jqwidgets.createInstance(".viewButton", "jqxButton", {
      height: 26,
      width: 200,
      value: "Send Reminder Notification"
    });
